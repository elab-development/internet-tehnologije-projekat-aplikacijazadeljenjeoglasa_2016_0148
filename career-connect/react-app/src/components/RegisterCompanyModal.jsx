...
function RegisterCompanyModal({ onClose }) {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!name) newErrors.name = 'Naziv kompanije je obavezan.';
    ...
    if (!website || !urlPattern.test(website)) newErrors.website = 'Websajt mora početi sa http:// ili https:// i biti validan URL.';
  ...
  const handleRegister = async () => {
    if (validateForm()) {
      try {
        ...
        // Pozovi API za registraciju kompanije
        ...
        setAlert({ message: 'Kompanija je uspešno registrovana!', type: 'success' });
